...
print('%s amonalies detected:'%(len(f:=['\n'.join([i]+[i+k for k in['.avif','.c.avif','.webp']if exists(i+k)])for i in incepts([getcwd()+sep],['*.jpg','*.jpeg','*.jfif','*.png','*.bmp'])if any([exists(i+j)for j in['.avif','.c.avif','.webp']])])),*f,'Please check these files manually',sep='\n\n')
