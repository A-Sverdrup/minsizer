...
Almost in-place conversion. Requires at most 8x (BMP/TIFF) / 7x (PNG) / 6x (JPG)
...
def convertPNG(path,c2=True):
    ...
    for b,i,o in [['cavif',f'--lossless -i "{path}" -o "{path}{ct2}.c.avif"',f'{path}{ct2}.c.avif'],
                  ...
                  ['cjxl',f'{path} {path}{ct2}.jxl -q 100 -e 10 -a {"0"if alpha else"25"}',f'{path}{ct2}.jxl'],
                  ['magick',f'"{path}" -quality 100 {""if alpha else "-alpha off "}-define avif:lossless=true "{path}{ct2}.m.avif"',f'{path}{ct2}.m.avif'],
                  ['magick',f'"{path}" -quality 100 {""if alpha else "-alpha off "}-define webp:lossless=true -define webp:method=6 "{path}{ct2}.m.webp"',f'{path}{ct2}.m.webp'],,
                  ['magick',f'"{path}" -quality 100 -define jxl:lossless=true -define jxl:effort=10 {path}.jxl',f'{path}.jxl']][int(alpha):]:
...
def convertJPG(path):
    return convert___(path,[['avifenc',f'--qalpha 0 -d 8 "{path}" -o "{path}.avif"',f'{path}.avif'],
                            ...
                            ['cjxl',f'{path} {path}.jxl -q 100 -e 10 -a 25',f'{path}.jxl'],
                            ...
                            ['magick',f'"{path}" -quality 100 -define jxl:lossless=true -define jxl:effort=10 {path}.jxl',f'{path}.jxl']],
                      {'.c.avif':'.avif','.m.avif':'.avif','.m.webp':'.webp'})
def convertGIF(path):
    return convert___(path,[['gif2webp',f'-min_size "{path}" -o "{path}.webp"',f'{path}.webp'],
                            ['cjxl',f'{path} {path}.jxl -q 100 -e 10 -a 0',f'{path}.jxl'],
