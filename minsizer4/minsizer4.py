...
def pause():input('Press Enter to continue (spam Ctrl-C to cancel and exit)...')
...
pf=lambda*a:print(*a,end=' ');c80=lambda s='':print('\n'+s.center(80,'-'));X=lambda:print('X',end=' ');V=lambda:print('V',end=' ')
def convert_2_(path,binary,fstring,ext):
    t=-time();pf(binary);I=getsizenan(path)
    if shell(fstring):t+=time();X();keep(path);return t,I,I
    else:
        t+=time();V()
        if(c:=convertPNG(path[:path.rindex('.')]+'.png',nl=None))[3]:
            if getsizenan(path[:path.rindex('.')]+'.png')<I:pf(f'minsized ({size(I,c[2])})',path[:path.rindex('.')]+'.png');rmerr(path);return t+c[0],I,c[2]
            else:keep(path);rmerr(path[:path.rindex('.')]+'.png');return t,I,I
        else:
            if c[2]<I:
                pf(f'minsized ({size(I,c[2])})')
                try:rename(c[4],(m:=c[4].replace('.png.c.avif',f'.{ext}.avif').replace('.png.avif',f'.{ext}.avif').replace('.png.webp',f'.{ext}.webp')));pf(m)
                except:pf(c[4])
                rmerr(path)
                return t+c[0],I,c[2]
            else:keep(path);rmerr(c[4]);return t,I,I
...
# DO NOT REMOVE convertPNG if you want to convert TIFF and BMP!
...
def convert(files):
    ...
    if mode:
        ...
        print('Automatic mode: %s Ignore errors: %s'%(mode,'NY'[int(auto)]))
        for i in range(5):print(f'Starting in {5-i} seconds. If you want to cancel, spam Ctrl-C NOW!',end='\r');sleep(1)
        print('Starting NOW!')
    else:
        c80(' Images found: ')
        for i in formats:
            if(l:=len(f:=fl[i])):print(f'[{" ".join(i)}]: {l} files ({size(sum(getsizenan(j,0)for j in f))})')
            else:print('[%s]: No files found'%(' '.join(i)))
        c80()
        ...
        if not auto:
            pause()
            auto=(input('Pause on errors (default=Y)? [Y/N]')in['n','N'])
    S=''
    for i in formats:
...
def extrargt(name,name2=None):
    if name2:return(extrargt(name)or extrargt(name2))
        ...
        del argv[argv.index(name)]
        return True
    else:return False
mode=None
for i in ['/MODE','-m','--mode']:
    if i in argv:
        mode=argv.pop(argv.index(i)+1)
