    ...
    if shell(fstring):t+=time();X();keep(path);return t,I,I
        ...
        if(c:=convertPNG(path[:path.rindex('.')]+'.png',nl=None))[3]:
            ...
            if c[2]<I:
                ...
                try:rename(c[4],(m:=c[4].replace('.png.nlc.avif',f'.{ext}.avif').replace('.png.nl.avif',f'.{ext}.avif').replace('.png.nl.webp',f'.{ext}.webp')));pf(m)
                except:pf(c[4])
                rmerr(path)
                return t+c[0],I,c[2]
...
def convertPNG(path,nl=True):
    ...
    if nl:
        BIO=[['cavif',f'--lossless -i "{path}" -o "{path}.c.avif"',f'{path}.c.avif'],
    ...
    for i in formats:
        try:
            if(l:=len(f:=fl[i]))and formats[i]:
                ...
                for j in f:
                    try:pf(f'{n}/{l}');t,Is,s=formats[i](j);T+=t;pe(f'{date(t,T*l/n-T)}');I+=Is;MS+=s;n+=1
